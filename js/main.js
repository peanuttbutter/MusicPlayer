...
renderCurrentItem({ link, track, artist, duration, year }) {
  ...
  <div class="current-info">
    <div class="current-info__top">
      <div class="current-info__titles">
        <h2 class="current-info__artist">${artist}</h2>
...
togglePlaying() {
  const { playing, current: { audio } } = this.state;
...
loadAudioData(audio) {
  this.audioList.insertAdjacentHTML( 'beforeend', this.renderItem(audio))
},

renderItem({id, link, track, artist, genre, duration}) {
  const [image] = link.split('.');
